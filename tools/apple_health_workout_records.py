...
def main() -> None:
    parser = argparse.ArgumentParser(description="Inspect Apple Health records around running workouts")
    parser.add_argument("--file", default="data/apple_health/export.xml")
    parser.add_argument("--limit", type=int, default=3)
    parser.add_argument("--window", type=int, default=60, help="minutes around workout start/end")
    ...
    # Re-read the file and collect interesting quantity records that overlap
    # the selected workout windows. This is intentionally streaming-friendly.
    selected = []
    ...
    records_by_workout: dict[int, dict[str, list[dict]]] = defaultdict(lambda: defaultdict(list))
    for event, elem in ET.iterparse(path, events=("end",)):
        if elem.tag == "Record":
            attrs = elem.attrib
